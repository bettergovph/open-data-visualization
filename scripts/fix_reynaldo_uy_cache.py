#!/usr/bin/env python3
"""
Special script to clean Reynaldo Uy's cache to Samar 1st District projects only.
"""
...
class ReynaldoUyCacheFixer:
    def __init__(self) -> None:
        ...
        self.target_name = 'Reynaldo Uy'
        self.slug = 'reynaldo-uy'
    ...
    def load_config(self) -> Dict:
        ...
        for entry in config.get('target_congressmen', []):
            if entry.get('display_name') == self.target_name:
                return entry
        raise RuntimeError('Configuration for Reynaldo Uy not found')
    ...
    def build_filters(self, districts_data: Dict) -> Dict[str, List[str]]:
        ...
        municipalities = samar_info.get('municipalities', {})
        target_municipalities = [name.upper() for name, dist in municipalities.items() if dist == '1st District']
        other_municipalities = [name.upper() for name, dist in municipalities.items() if dist != '1st District']
    ...
    def filter_projects(self, projects: List[Dict], filters: Dict[str, List[str]]) -> List[Dict]:
        ...
        for proj in projects:
            ...
            if not mentioned_targets and not mentioned_others:
                if any(keyword in combined for keyword in positives) and 'SAMAR' in combined:
                    filtered.append(proj)
    ...
    def run(self) -> None:
        ...
        print('✅ Reynaldo Uy cache updated successfully')

if __name__ == '__main__':
    ReynaldoUyCacheFixer().run()
