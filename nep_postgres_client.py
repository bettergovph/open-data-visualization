...
DB_CONFIG = {
    ...
    'database': os.getenv('POSTGRES_DB', 'budget_analysis').split(',')[0],  # Use first database from comma-separated list
...
async def execute_budget_query(query: str, year: str = "2025"):
    ...
    try:
        ...
        for row in rows:
            row_dict = dict(row)
            # Convert all Decimal values to float and datetime to string for JSON serialization
            for key, value in row_dict.items():
                if hasattr(value, '__class__') and 'Decimal' in str(type(value)):
                    row_dict[key] = float(value)
                elif hasattr(value, '__class__') and 'datetime' in str(type(value)).lower():
...
async def get_budget_overview_stats(year: str = None):
    """Get overview statistics for NEP data - optionally filter by year"""
    try:
        print("🔍 [PostgreSQL] Getting NEP overview stats")
        ...
        if year and year.isdigit() and year in ['2020', '2021', '2022', '2023', '2024', '2025', '2026']:
            ...
            SELECT
                ...
                COUNT(DISTINCT org_uacs_code) as unique_departments,
                COUNT(DISTINCT region_code) as unique_regions
            FROM {table_name}
            WHERE amount IS NOT NULL AND amount > 0
            """
        else:
            # Get total items and value across all NEP years (2020-2026)
            overview_query = """
            SELECT
                COUNT(*) as total_items,
                COALESCE(SUM(amount), 0) as total_value,
                ...
                COUNT(DISTINCT region_code) as unique_regions
            FROM (
                SELECT amount, org_uacs_code, region_code FROM budget_2026
                UNION ALL
                SELECT amount, org_uacs_code, region_code FROM budget_2025
                UNION ALL
                SELECT amount, org_uacs_code, region_code FROM budget_2024
                UNION ALL
                SELECT amount, org_uacs_code, region_code FROM budget_2023
                UNION ALL
                SELECT amount, org_uacs_code, region_code FROM budget_2022
                UNION ALL
                SELECT amount, org_uacs_code, region_code FROM budget_2021
                UNION ALL
                SELECT amount, org_uacs_code, region_code FROM budget_2020
            ) combined_nep
            """

        result = await conn.fetchrow(overview_query)

        # Get top department by total amount (using org_uacs_code as department identifier)
        if year and year.isdigit() and year in ['2020', '2021', '2022', '2023', '2024', '2025', '2026']:
            # Filter by specific year
            top_dept_query = f"""
            SELECT
                org_uacs_code as department,
                org_uacs_code as dept_code,
                SUM(amount) as total_amount
            FROM budget_{year}
            WHERE org_uacs_code IS NOT NULL AND amount IS NOT NULL AND amount > 0
            GROUP BY org_uacs_code
            ORDER BY total_amount DESC
            LIMIT 1
            """
        else:
            # Get top department across all years
            top_dept_query = """
            SELECT
                org_uacs_code as department,
                org_uacs_code as dept_code,
                SUM(amount) as total_amount
            FROM (
                SELECT org_uacs_code, amount FROM budget_2026
                UNION ALL
                SELECT org_uacs_code, amount FROM budget_2025
                UNION ALL
                SELECT org_uacs_code, amount FROM budget_2024
                UNION ALL
                SELECT org_uacs_code, amount FROM budget_2023
                UNION ALL
                SELECT org_uacs_code, amount FROM budget_2022
                UNION ALL
                SELECT org_uacs_code, amount FROM budget_2021
                UNION ALL
                SELECT org_uacs_code, amount FROM budget_2020
            ) dept_totals
        ...
        top_dept = await conn.fetchrow(top_dept_query)

        # Get last updated timestamp
        last_updated_query = """
        SELECT MAX(updated_at) as last_updated
        FROM (
            SELECT updated_at FROM budget_2026
            UNION ALL
            SELECT updated_at FROM budget_2025
            UNION ALL
            SELECT updated_at FROM budget_2024
            UNION ALL
            SELECT updated_at FROM budget_2023
            UNION ALL
            SELECT updated_at FROM budget_2022
            UNION ALL
            SELECT updated_at FROM budget_2021
            UNION ALL
            SELECT updated_at FROM budget_2020
        ) timestamps
        """

        last_updated = await conn.fetchval(last_updated_query)
        ...
        return {
            ...
            "stats": {
                ...
                "top_department": top_dept['dept_code'] if top_dept else "INVALID",
                "top_dept_amount": float(top_dept['total_amount']) if top_dept else 0,
                "unique_departments": result['unique_departments'],
                "unique_agencies": result['unique_regions'],  # Using regions as agencies for NEP
...
async def get_budget_data_browser_all_years(years: list, page: int = 1, limit: int = 50, sort_by: str = "amount", sort_order: str = "DESC", filters: dict = None):
    """Get paginated NEP data across all years with sorting and column filtering"""
    try:
        print(f"🔍 [PostgreSQL] Getting NEP data browser across all years, page {page}, limit {limit}, sort by {sort_by} {sort_order}")
        ...
        # Validate sort parameters
        allowed_sort_columns = ['org_uacs_code', 'description', 'fiscal_year', 'region_code', 'funding_uacs_code', 'object_uacs_code', 'amount', 'year']
        if sort_by not in allowed_sort_columns:
            sort_by = 'amount'
        ...
        if filters:
            if 'department' in filters and filters['department']:
                where_conditions.append("org_uacs_code = $1")
                params.append(filters['department'])

            if 'agency' in filters and filters['agency']:
                where_conditions.append("region_code = $" + str(len(params) + 1))
            ...
            if 'expense_category' in filters and filters['expense_category']:
                where_conditions.append("object_uacs_code = $" + str(len(params) + 1))
                params.append(filters['expense_category'])

            if 'text_filter' in filters and filters['text_filter']:
                where_conditions.append("description ILIKE $" + str(len(params) + 1))
                params.append(f"%{filters['text_filter']}%")

            if 'amt_min' in filters and filters['amt_min'] is not None:
                where_conditions.append("amount >= $" + str(len(params) + 1))
        ...
        print(f"🔍 [PostgreSQL] Executing NEP query with {len(params)} parameters")
        ...
        print(f"✅ [PostgreSQL] Retrieved {len(data)} NEP rows across {len(years)} years, total: {total_count}")
        return result
