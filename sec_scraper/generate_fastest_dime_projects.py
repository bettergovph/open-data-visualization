...
async def generate_slowest_dime_projects():
    """Generate slowest completed DIME projects from database"""
    conn = await get_db_connection()
    if not conn:
        return None
    
    try:
        # Get slowest completed projects (top 100, sorted by completion days descending then cost descending)
        query = """
        ...
        ORDER BY completion_days DESC, cost DESC
        LIMIT 100
        """
        ...
        return {
            ...
            'description': 'Top 100 slowest completed DIME projects (sorted by completion days descending, then cost descending)',
            'cache_version': '1.0'
        }
        
    except Exception as e:
        print(f"❌ Error generating slowest DIME projects: {e}")
        await conn.close()
        return None
...
async def main():
    """Generate fastest and slowest DIME projects JSON files"""
    print("⚡ Generating Fastest and Slowest DIME Projects...")
    
    # Generate fastest projects
    print("\n🏃 Generating fastest projects...")
    fastest_data = await generate_fastest_dime_projects()
    
    if not fastest_data:
        print("❌ Failed to generate fastest DIME projects data")
        return
    
    # Generate slowest projects
    print("\n🐌 Generating slowest projects...")
    slowest_data = await generate_slowest_dime_projects()
    
    if not slowest_data:
        print("❌ Failed to generate slowest DIME projects data")
        return
    
    # Create output directory if it doesn't exist
    os.makedirs('static/data', exist_ok=True)
    
    # Save fastest projects to JSON file
    fastest_output_file = 'static/data/fastest_dime_projects.json'
    with open(fastest_output_file, 'w', encoding='utf-8') as f:
        json.dump(fastest_data, f, indent=2, ensure_ascii=False)
    
    # Save slowest projects to JSON file
    slowest_output_file = 'static/data/slowest_dime_projects.json'
    with open(slowest_output_file, 'w', encoding='utf-8') as f:
        json.dump(slowest_data, f, indent=2, ensure_ascii=False)
    
    print(f"\n✅ Generated {fastest_output_file}")
    print(f"📊 Fastest Projects: {fastest_data['count']}")
    if fastest_data['projects']:
        fastest = min(fastest_data['projects'], key=lambda x: x['completion_days'] or float('inf'))
        print(f"🏆 Fastest: {fastest['completion_days']:.1f} days - {fastest['project_name'][:50]}...")
    
    print(f"\n✅ Generated {slowest_output_file}")
    print(f"📊 Slowest Projects: {slowest_data['count']}")
    if slowest_data['projects']:
        slowest = max(slowest_data['projects'], key=lambda x: x['completion_days'] or 0)
        print(f"🐌 Slowest: {slowest['completion_days']:.1f} days - {slowest['project_name'][:50]}...")
