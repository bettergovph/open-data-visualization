#!/usr/bin/env python3
"""
2025 Election Data Import Script - MEMORY-BASED APPROACH
Load all data into memory and process efficiently
"""
...
async def main():
    ...
    print(f"🧠 Loaded {len(all_data)} records into memory")
    ...
    for position, records in by_position.items():
        print(f"📊 Processing {len(records)} records for {position}...")
        ...
        if position in ['SENATOR', 'MEMBER, HOUSE OF REPRESENTATIVES']:
            ...
            for record in records:
                key = f"{record['first_name']}|{record['last_name']}"
                candidate_votes[key] += record['votes']
                candidate_info[key] = record  # Store the last record info
            ...
            for record in records:
                location_key = f"{record['province']}|{record['municipality_city']}"
                location_candidates[location_key].append(record)
            ...
            for location_key, location_records in location_candidates.items():
                ...
                for record in location_records:
                    key = f"{record['first_name']}|{record['last_name']}"
                    location_candidate_votes[key] += record['votes']
                    location_candidate_info[key] = record
