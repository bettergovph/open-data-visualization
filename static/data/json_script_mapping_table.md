...
| `fastest_dime_projects.json` | API endpoint (generated on-demand) | DIME Data | DIME database |
...
- `sec_scraper/generate_contractors_dashboard_cache.py` → 1 JSON file
- `utils/generate_summary_stats.py` → 4 JSON files
- `analysis/flood_same_amount_proximity_analysis.py` → 1 JSON file

### API Endpoints (Generated on-demand)
- 19 JSON files generated by API endpoints
- These are cached in memory or generated on-demand for performance
