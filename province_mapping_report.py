#!/usr/bin/env python3
"""
Province Mapping Report: Dynasty Database vs GeoJSON Files
This script creates a comprehensive mapping between dynasty database provinces and GeoJSON files.
"""
...
def normalize_province_name(name: str) -> str:
    ...
    variations = {
        ...
        'ncr, city of manila, second district': 'manila', 
        'ncr, city of manila, third district': 'manila',
        'ncr, city of manila, fourth district': 'manila',
        'ncr, first district': 'manila',
        'ncr, second district': 'manila', 
        'ncr, third district': 'manila',
        'ncr, fourth district': 'manila',
...
def extract_geojson_province(filename: str) -> str:
    ...
    # Remove .geo.json extension
    name = basename.replace('.geo.json', '')
    # Extract province from ph.region.province.any.any format
    parts = name.split('.')
    if len(parts) >= 3:
...
def create_province_mapping():
    """Create comprehensive province mapping"""
    
    # Dynasty database provinces (from the query result)
    dynasty_provinces = [
    ...
    # GeoJSON province files (from the ls command result)
    geojson_files = [
    ...
    # Try to match each dynasty province with a GeoJSON file
...
def generate_report():
    ...
    result = create_province_mapping()
    
    print("=" * 80)
    print("🏛️ PROVINCE MAPPING REPORT: Dynasty Database vs GeoJSON Files")
    print("=" * 80)
    print()
